...
pub async fn list_supply_requests(
    ...
    // Join projection table with supply_requests for order_ids
    ...
    Ok(Json(
        rows.into_iter()
            .map(|(id, description, state_str, order_ids_json)| {
                let order_ids: Vec<Uuid> = serde_json::from_value(order_ids_json)
                    .unwrap_or_default();
...
/// D08-4: POST /branches/:branch_id/supply-requests/:supply_request_id/send
/// Owner dispatches Draft SR to Supplier — appends SupplyRequestSent event.
pub async fn send_supply_request(
    AuthorizedBranch { branch_id, .. }: AuthorizedBranch,
    Path(params): Path<std::collections::HashMap<String, String>>,
    State(state): State<AppState>,
) -> Result<StatusCode, (StatusCode, String)> {
    let supply_request_id = params
        .get("supply_request_id")
