//! T04: `POST /webhooks/line`, `GET`/`POST /webhooks/whatsapp`. Each handler
//! verifies, dedups into `webhook_inbox`, and returns immediately — R01/R02
//! both say ack fast and process async, so nothing here calls `agent`.
...
pub async fn line_webhook(State(state): State<AppState>, headers: HeaderMap, body: Bytes) -> impl IntoResponse {
    ...
    match messaging::inbound::receive(state.line.as_ref(), Channel::Line, &state.inbox, &h, &body).await {
        Ok(messages) => {
            // T04: the only legitimate use of a reply token — a synchronous
            // in-handler ack, sent before returning. Agent/Owner-triggered
            // content always goes through push instead (inbox_worker.rs).
            for msg in messages {
...
pub async fn whatsapp_webhook(State(state): State<AppState>, headers: HeaderMap, body: Bytes) -> impl IntoResponse {
    let h = to_messaging_headers(&headers);
