...
pub async fn render_suppliers(
    ...
    let current_path = format!("/branches/{branch_id}/suppliers");
    ...
    {
        Ok(r) => r,
        Err(e) => { eprintln!("suppliers query failed: {e}"); return page_not_found(); }
    };
    ...
    let rows_html = if rows.is_empty() {
        ...
        rows.iter()
            .map(|(id, name, channel, external_id)| {
                supplier_row_html(*id, name, channel.as_deref(), external_id.as_deref(), &t)
            })
            .collect::<Vec<_>>()
            .join("\n")
    };
...
<div class="page">
  <div class="page-header">
    <h1>{title}</h1>
    <button class="btn btn--primary" onclick="BB.openModal('create-supplier-modal')">{add_btn}</button>
  </div>
  ...
  <div class="modal">
    ...
    <div class="modal__body">
      <div class="form-group">
        <label class="form-label" for="supplier-name">{name_label}</label>
    ...
    <div class="modal__footer">
      ...
      <button class="btn btn--primary" id="create-supplier-btn">{create_btn}</button>
    </div>
  </div>
</div>
...
{shell_close}"#,
        shell_open     = shell_open(t.get("suppliers.page_title"), t.lang()),
        topbar         = topbar_html(branch_id, &branch_name, &all_branches, "suppliers", &t, &current_path),
        title          = t.get("suppliers.title"),
        add_btn        = t.get("suppliers.add"),
        onboard_heading= t.get("suppliers.onboarding.heading"),
        step1          = t.get("suppliers.onboarding.step1"),
        step2          = t.get("suppliers.onboarding.step2"),
        step3_pre      = t.get("suppliers.onboarding.step3_pre"),
        step3_link     = t.get("suppliers.onboarding.step3_link"),
        step3_post     = t.get("suppliers.onboarding.step3_post"),
        step4          = t.get("suppliers.onboarding.step4"),
        actors_href    = actors_href,
        col_name       = t.get("suppliers.col.name"),
        col_channel    = t.get("suppliers.col.channel"),
        ...
        hint           = t.get("suppliers.create.hint"),
        cancel         = t.get("btn.cancel"),
        create_btn     = t.get("suppliers.create.btn"),
        branch_id      = branch_id,
...
fn supplier_row_html(
    ...
    let (binding_cell, channel_cell) = match (channel, external_id) {
        ...
        _ => (
            format!(r#"<span class="text-muted text-xs">{}</span>"#, t.get("suppliers.not_bound")),
  ...
  <td>{binding_cell}</td>
  <td>{channel_cell}</td>
  <td><button class="btn btn--ghost btn--sm" onclick="supplierDelete('{id}')">{remove}</button></td>
</tr>"#,
        id           = id,
